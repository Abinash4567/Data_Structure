...
bool child_parent_sum(Node* root)
{
	if(root==NULL)
		return true;
	...
	if(check and child_parent_sum(root->left) and child_parent_sum(root->right))
		return true;
	else
		return false;
...
int main()
{
	...
	if(child_parent_sum(root))
		cout<<"sum parent balanced.";
	else
		cout<<"Not sum parent balanced.";
